<script lang="ts">
	...
	// The API URL and the query string are only known in the browser.
	let activeTag = $state('');
	let announcements = $state<Promise<Announcement[]>>();
	$effect(() => {
		const tag = page.url.searchParams.get('tag')?.trim() ?? '';
		activeTag = tag;
		announcements = fetchAnnouncements(settings.apiUrl, tag);
	});
</script>

<PageMeta
	title="Announcements · Reseam"
	description="The latest Reseam announcements: releases, notices, and community news."
/>
